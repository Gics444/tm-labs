	...
	for bodies in collecting_area.get_overlapping_areas():
		if bodies.is_in_group("Shape_essence"):
			...
			if se.assigned_worker == null and se.valid == true:
				if worker1.shape_target == worker1.worker_poz.global_position or worker1.shape_target == null:
					free_worker = worker1
				elif worker2.shape_target == worker2.worker_poz.global_position or worker2.shape_target == null:
					free_worker = worker2
				elif worker3.shape_target == worker3.worker_poz.global_position or worker3.shape_target == null:
					free_worker = worker3
				else:
					free_worker = null
				if free_worker != null:
